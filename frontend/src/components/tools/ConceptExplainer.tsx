...
import { explainConcept, SourceItem } from "@/lib/api";

interface ConceptExplainerProps {
  sessionId: string;
}

export function ConceptExplainer({ sessionId }: ConceptExplainerProps) {
      ...
      <div className="p-6 border-b border-white/5 bg-[#050505]/40 backdrop-blur-xl shrink-0 relative z-20 overflow-hidden">
        ...
        <div className="flex flex-wrap items-end gap-4 max-w-4xl relative z-10">
          ...
          <div className="space-y-1.5 min-w-[180px]">
            <label className="text-[10px] font-semibold text-neutral-500 uppercase tracking-[0.2em] font-mono">Topic (optional)</label>
            <input
              type="text"
      ...
      <div className="flex-1 p-6 md:p-8 flex flex-col items-center relative z-10 w-full max-w-4xl mx-auto">
        ...
        {explanationData && (
          <div className="w-full max-w-3xl animate-enter space-y-6 pb-20">
              ...
              <div className="bg-white/2 p-6 md:p-8 border-b border-white/5 flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div>
                  <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    {(explanationData.beginner_mode || isBeginner) && (
                    ...
                    <span className="px-2 py-1 bg-purple-500/10 rounded-sm text-[10px] uppercase tracking-widest text-purple-400 border border-purple-500/20 font-mono">
                      {explanationData.difficulty || difficulty}
                    </span>
                  </div>
            ...
            {/* Sources Citations */}
            {Array.isArray(explanationData.sources) && explanationData.sources.length > 0 && (
              <div className="bg-[#0a0a0a] border border-white/10 rounded-xl p-6 shadow-lg">
                ...
                <div className="space-y-4">
                  {explanationData.sources.map((node: SourceItem, idx: number) => (
                    <div key={idx} className="p-5 bg-black/40 border border-white/5 hover:border-blue-500/30 rounded-lg transition-colors group">
                      <div className="flex flex-wrap gap-4 text-[10px] font-mono tracking-widest text-neutral-500 uppercase">
                        <span className="flex items-center gap-1.5"><span className="text-blue-500/80">Page:</span> <span className="text-neutral-300">{node.page ?? 'Unknown'}</span></span>
                      </div>
                    </div>
                  ))}
                </div>
