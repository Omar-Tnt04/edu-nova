...
import { generateFlashcards, SourceItem } from "@/lib/api";
...
export function FlashcardTool({ sessionId, demoTopic, demoDifficulty, demoRunId = 0 }: FlashcardToolProps) {
          ...
          <div className="w-full max-w-2xl flex flex-col items-center space-y-8 animate-enter duration-500">
            ...
            <div className="w-full aspect-[3/2] [perspective:1000px] cursor-pointer group" onClick={() => setIsFlipped(!isFlipped)}>
              <motion.div
                ...
                animate={{ rotateY: isFlipped ? 180 : 0, scale: isFlipped ? 1.01 : 1 }}
                transition={{ type: "spring", stiffness: 260, damping: 20 }}
              >
                {/* Front */}
                <div
                  className="absolute inset-0 bg-black/60 backdrop-blur-md border border-white/10 rounded-2xl flex flex-col items-center justify-center text-center p-8 group-hover:border-blue-500/30 transition-colors overflow-hidden"
                ...
                {/* Back */}
                <div
                  className="absolute inset-0 bg-blue-950/20 backdrop-blur-md border border-blue-500/20 rounded-2xl p-8 flex flex-col items-center justify-center text-center overflow-y-auto shadow-[0_0_30px_rgba(59,130,246,0.1)]"
                  style={{
            ...
            <div className="flex gap-4">
              ...
              <span className="text-xs text-neutral-600 tracking-wide uppercase">Tip: click card to flip</span>
            </div>

            {Array.isArray(cards[currentIndex]?.sources) && cards[currentIndex].sources.length > 0 && (
              <div className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg p-4">
                <p className="text-[10px] font-mono text-neutral-500 uppercase tracking-[0.2em] mb-3">Sources</p>
                <div className="space-y-2">
                  {cards[currentIndex].sources.map((source: SourceItem, sourceIdx: number) => (
            ...
            <div className="text-xs text-neutral-500 uppercase tracking-[0.2em] font-mono">
