...
function LoginForm() {
  ...
  useEffect(() => {
    if (searchParams.get("signup_success")) {
      setMessage("Account created successfully. Please check your email to verify (if enabled) or log in.");
    }
  }, [searchParams]);
  ...
  const handleGoogleAuth = async () => {
    ...
    setError('');
    setTimeout(() => {
      // Assume student by default for Google
      localStorage.setItem("access_token", "mock_token_google");
      localStorage.setItem("user", JSON.stringify({ email: "google_user@example.com", role: "student" }));
      router.push("/chat");
    }, 500);
  };
  ...
  return (
    <main className="min-h-screen bg-[#050505] flex flex-col justify-center relative overflow-hidden font-sans selection:bg-blue-500/30">
      ...
      <div className="max-w-md w-full mx-auto p-6 md:p-8 relative z-10">
        ...
        >
          <h1 className="text-3xl font-display font-medium text-white mb-2 tracking-tight mt-4">Welcome back</h1>
          <p className="text-neutral-400 font-light">Login with your Google account or email</p>
        </motion.div>

        {message && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 p-4 rounded-lg mb-6 text-sm text-center">
            {message}
          </motion.div>
        )}
        ...
        >
          ...
          <div className="relative py-6">
            ...
            <div className="relative flex justify-center text-xs uppercase cursor-default">
              <span className="bg-[#050505] px-4 text-neutral-500 tracking-widest">Or continue with</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <button type="button" className="text-xs text-blue-400 hover:text-blue-300 transition-colors">Forgot your password?</button>
              </div>
              <input
                type="password"
                required
